...
{
	int offset=blockIdx.x*blockDim.x/32+threadIdx.x/32;
	int offset_in_block=threadIdx.x/32;
	int4 result4;;
	short2 * direction_index;
	__shared__ char * cigar_store[4];
	__shared__ int *cigar_int_store[4];
	__shared__ char cigar_m[128*4];
        __shared__ int cigar_int_m[128*4];
	while(offset<size)
         {
		 char state;//0  match;  1 mistmatch; 2 inseriton;  3  deletion
               	 int cigar_index;
               	__shared__ int cigar_number[4];
		 int segment_length;
                short2 btr;
                char  new_state;
    	        int step_length;
		...
		if( threadIdx.x%32==0)
			...
			direction_index=(short2 *) (direction+offset*640*1100);  
			cigar_store[offset_in_block]=(char *) (cigar+offset*sizeof(char)*128);
			cigar_int_store[offset_in_block]=(int *) (cigar_int+offset*128);
		...
		//	printf("%d %d\n", result4.x,result4.y);
			...
			if(result4.z>0)
			{
			cigar_m[128*offset_in_block+cigar_index]='S';
			cigar_int_m[128*offset_in_block+cigar_index]=result4.z;
			cigar_index++;
			}
			segment_length=0;
			state='N';
			do
			{
				...
				{
				 cigar_m[128*offset_in_block+cigar_index]=state;
                       		 cigar_int_m[128*offset_in_block+cigar_index]=segment_length;
                       		 segment_length=step_length;
                       		 cigar_index++;
			 	 state=new_state;
				}
			...
			{	
				cigar_m[128*offset_in_block+cigar_index]='S';
				cigar_int_m[128*offset_in_block+cigar_index]=result4.y+1;
				cigar_index++;
			}	
	...
	//		for(int i=cigar_index-1;i>=0;i--)
		//	{
	//		printf("%d%c",cigar_int_m[128*offset_in_block+i],cigar_m[128*offset_in_block+i]);
		//	}
	//		printf("\n");
	
			cigar_number[offset_in_block]=cigar_index;
		 }
		 __syncthreads();
	

		int tt=(cigar_number[offset_in_block]+31)/32;
		int offset_in_warp=threadIdx.x%32;
		for(int i=0;i<tt;i++)		
		{
	//	printf("%d %d ",i,threadIdx.x,offset_in_warp);
		if(offset_in_warp+i*32<cigar_number[offset_in_block])
		{
	//	printf("size=%d offset=%d %d %d \n",size, offset, threadIdx.x,cigar_number[offset_in_block]-1-i*32-offset_in_warp, 128*offset_in_block+i*32+offset_in_warp);
		(cigar_store[offset_in_block])[cigar_number[offset_in_block]-1-i*32-offset_in_warp]=cigar_m[128*offset_in_block+i*32+offset_in_warp];
		(cigar_int_store[offset_in_block])[cigar_number[offset_in_block]-1-i*32-offset_in_warp]=cigar_int_m[128*offset_in_block+i*32+offset_in_warp];
		}
		}

		 __syncthreads();
		offset+=gridDim.x*blockDim.x/32;
	}
}
...
int main(int artc, char* args[])
{
	...
	timespec start,finish;	 
	 ...
	 while(!feof(file))
		...
		dim3 block1(128);
		dim3 grid1(size/4+1);
		calculate_cigar_2<<<grid1,block1>>> (size,result_d,cigar,cigar_int,direction); //result
