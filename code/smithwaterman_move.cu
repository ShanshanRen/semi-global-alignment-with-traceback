	...
	while(offset<size)
		...
		int read_number=read_reference_number.x;
			...
			int current_reference_id=0;
					...
					gap_v+=extend;
//				 if(threadIdx.x==47&&current_reference_id==72||threadIdx.x==46&&current_reference_id==71)
...
//if(threadIdx.x==47&&current_reference_id==72||threadIdx.x==46&&current_reference_id==71)
        //printf("%d %d %d %d %d %d   %d %d\n",640*j+threadIdx.x,current_reference_id, threadIdx.x,M,step_right,gap_v,curmt.x,curmt.y);   			
					MMM=MM[threadIdx.x];
		 ...
		 if(threadIdx.x==read_reference_number.x-1)
			...
			do
			{
				btr=direction_index[(final_i+final_j)*640+final_j];
				 ...
				 //printf("%d %d %d  %d %d %d\n",(final_i+final_j)*640+final_j,cigar_index,final_i,final_j,btr.x,btr.y);
