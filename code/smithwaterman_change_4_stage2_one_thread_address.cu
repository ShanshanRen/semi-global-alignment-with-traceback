  ...
  __global__ void  calculate_cigar_2( int size, int4 * result, char * cigar,int * cigar_int,int * direction) //, char * result
{
	...
	while(offset<size)
		...
		//if( threadIdx.x==0)
			...
			int d=offset/blockDim.x * blockDim.x*128;
			direction_index=(short2 *) (direction+offset*640*1100);  
			cigar_store=(char *) (cigar+d);
			cigar_int_store=(int *) (cigar_int+d);
			...
			cigar_store[threadIdx.x+cigar_index*128]='S';
			cigar_int_store[threadIdx.x+cigar_index*128]=result4.z;
			cigar_index++;
			}
			segment_length=0;
			state='N';
			do
			{
				...
				// cigar_m[cigar_index]=state;
                       		...
                       		cigar_store[threadIdx.x+cigar_index*128]=state;
				cigar_int_store[threadIdx.x+cigar_index*128]=segment_length;
				segment_length=step_length;
                       		 cigar_index++;
			 	 state=new_state;
				}
		...
		//	cigar_m[cigar_index]=state;
			...
			//	cigar_int_m[cigar_index]=result4.y+1;
				cigar_store[threadIdx.x+cigar_index*128]='S';
				cigar_int_store[threadIdx.x+cigar_index*128]=result4.y+1;
				cigar_index++;
			}	
	...
	timespec start,finish;	 
	 ...
	 while(!feof(file))
		...
		for(int i=0;i<size;i++)
		{
			...
			{
			if(j!=result_h[i*4+3]-1) printf(", ");
			printf("%d%c",cigar_int_h[d+j*128+i%128],cigar_h[d+j*128+i%128]);
			}
			printf("]\n");
		}
