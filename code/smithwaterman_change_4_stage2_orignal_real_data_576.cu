  ...
  __global__ void  calculate_cigar( int size, char * data, NUM_ADD *num_add,int4 * result, int * direction) //, char * result
{
	...
	while(offset<size)
		...
		__shared__ int MM[577]; 
                __shared__ int gap_h[577]; //insertion
                __shared__ short2 gap_size_h[577];  //insertion
...
int main(int artc, char* args[])
{
	...
	double  computation_time2=0;//total_time=0;
	timespec start,finish;	 
     /*  char data[200][1000];
       ...
       */
		...
		cudaMalloc( (int **) & direction,  1200 * (640*1200* sizeof (int)));
...
cudaDeviceSynchronize();
  //              computation_time+=diff(start,finish);
		...
		dim3 block1(128);
//		clock_gettime(CLOCK_MONOTONIC_RAW,&start);
		calculate_cigar_2<<<grid,block1>>> (size,result_d,cigar,cigar_int,direction); //result
//		 cudaDeviceSynchronize();
//        	clock_gettime(CLOCK_MONOTONIC_RAW,&finish);
	...
	*/	
		...
		free(cigar_int_h);
		free(data_h_total);
                computation_time2+=diff(start,finish);
	       fscanf(file,"%d",&size);
        }

		cudaFree(direction);
 //	printf(" computation_time= %e  total_time=%e \n",computation_time,0);
printf(" computation_time= %e computation_time= %e   %d GCUPs=%lf\n",computation_time,computation_time2,total_size,( (double)total_size)/computation_time2/1000000000);
