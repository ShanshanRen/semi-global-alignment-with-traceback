  ...
  __global__ void  calculate_cigar( int size, char * data, NUM_ADD *num_add,int * result, char * cigar,int * cigar_int,int * direction) //, char * result
{
	...
	__shared__ char4 * direction_index;
	...
	while(offset<size)
		...
		direction_index=(char4 *) (direction+offset*640*1100);  
		...
		int read_number=read_reference_number.x;
			...
			int current_reference_id=0;
				...
				int aa=j-threadIdx.x;
					...
					gap_v+=extend;
		//		if(threadIdx.x==47&&current_reference_id==72||threadIdx.x==47&&current_reference_id==71||threadIdx.x==47&&current_reference_id==73)
	//printf("%d %d  open=%d extend= %d  \n",current_reference_id, threadIdx.x,prev_gap,gap_v);
					if(gap_v>=prev_gap) curmt.x='T';
					if(prev_gap>gap_v)
					{
						gap_v=prev_gap;
					}
				...
				//	curmt.x='T';	
					...
					if(step_right>=prev_gap) curmt.y='T';
...
//if(threadIdx.x==47&&current_reference_id==72||threadIdx.x==47&&current_reference_id==71||threadIdx.x==47&&current_reference_id==73)
	//printf("%d %d M=%d step_right=%d gap_v=%d %c %c  \n",current_reference_id, threadIdx.x,M,step_right,gap_v,curmt.x,curmt.y);
				if(M==gap_v||M>gap_v&&M>step_right)
				curmt.w=1;
				if(M==step_right&&M!=gap_v||M>gap_v&&M>step_right)
				curmt.z=1;
...
//if(threadIdx.x==47&&current_reference_id==72||threadIdx.x==47&&current_reference_id==71||threadIdx.x==47&&current_reference_id==73)	
//printf("%d %d %d %d %d %d %c %c  %c %c\n",640*j+threadIdx.x,current_reference_id, threadIdx.x,M,step_right,gap_v,curmt.x,curmt.y,curmt.z,curmt.w);
					MMM=MM[threadIdx.x];
		 ...
		 if(threadIdx.x==read_reference_number.x-1)
			...
			do
			{
				btr=direction_index[(final_i+final_j)*640+final_j];
				 //printf("%d %d %d  %d %c %c %c %c\n",(final_i+final_j)*640+final_j,cigar_index,final_i,final_j,btr.x,btr.y,btr.z,btr.w);	
				if(state=='D')
				{
					if(btr.x=='T')
					{
				...
				else
				{
				 cigar_m[cigar_index]=state;
                       		 cigar_int_m[cigar_index]=segment_length;
                       		if(new_state=='N') segment_length=0;
