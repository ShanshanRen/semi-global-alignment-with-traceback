...
void matrix(char * read, char * ref,int col,int row,int sw[][600],int btrack[][600])
{
    ...
    int best_gap_v[600],gap_size_v[600],best_gap_h[600],match_size_1[600],match_size_2[600];
...
//    		int ki=gap_size_h[i];
    		...
    		if(diag)
    		{
    			...
    			curBtrack[j]=match_size_1[j-1]+1;
			match_size_2[j]=match_size_1[j-1]+1;
    		}
    		else
    			...
    			else
    			{
    				...
    				curBtrack[j]=-kd; // negetive=vertical
    				match_size_2[j]=0;
			}
    	}
    }
...
void calculateCigar(int col,int row,int sw[][600],int btrack[][600])
{
        ...
        if ( btr > 0 ) 
        {
            ...
            step_length = btr;
        ...
        else if ( btr < 0 ) 
		        {
		        	...
		        	new_state = INSERTION; // and step_length =1, already set above

	printf("state=%d p1=%d p2=%d new_state=%d btr=%d\n",state,p1, p2,new_state,btr);
            // move to next best location in the sw matrix:
        switch( new_state ) 
        {
                case MATCH:  p1-=step_length; p2-=step_length; break; // move back along the diag in the sw matrix
                case INSERTION: p2 --; break; // move left
...
void smithwaterman(int size,InputData * inputdata)
{
	...
	for(int i=0;i<size;i++)
	{
        ...
        int btrack[600][600];
        //printf("%d\n",i);
		matrix(inputdata[i].read_base,inputdata[i].reference_base,col,row,sw,btrack);
		calculateCigar(col,row,sw,btrack);
	}
}
