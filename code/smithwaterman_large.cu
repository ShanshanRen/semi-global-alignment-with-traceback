  ...
  __global__ void  calculate_cigar( int size, char * data, NUM_ADD *num_add,int * result, char * cigar,int * cigar_int,int * direction) //, char * result
{
	...
	while(offset<size)
		...
		direction_index=(short2 *) (direction+offset*800*1100);  
		cigar_store=(char *) (cigar+offset*sizeof(char)*640);
		cigar_int_store=(int *) (cigar_int+offset*640);
	     }
                __syncthreads();
		...
		__shared__ int MM[645]; 
                __shared__ int gap_h[645]; //insertion
                __shared__ short2 gap_size_h[645];  //insertion
		...
		__shared__ char cigar_m[645];
		__shared__ int cigar_int_m[645];
		...
		for(int i=0;i<round;i++)
			...
			int current_reference_id=0;
					...
					direction_index[800*j+threadIdx.x]=curmt;
	...
	timespec start,finish;	 
       //printf("size=%d\n",size);
		...
		int * result_h=(int *) malloc(sizeof(int)*size*3);
	 	char * cigar_h=(char *) malloc(sizeof(char)*size*640);
		int * cigar_int_h=(int *) malloc(sizeof(int)*size*640);
		for(int i=0;i<size;i++)
		{
		...
		char * cigar;
		cudaMalloc( (char **) &cigar, size * (640* sizeof (char)+640*sizeof(int)));
		
		int * cigar_int=(int *) (cigar+size*640*sizeof(char));
		int * direction;
		...
		cudaMemcpy(cigar_h,cigar,640*sizeof(char)*size, cudaMemcpyDeviceToHost);
		cudaMemcpy(cigar_int_h,cigar_int,640*sizeof(int)*size,cudaMemcpyDeviceToHost);		
		...
		for(int i=0;i<size;i++)
		{
			...
			{
			if(j!=0) printf(", ");
			printf("%d%c",cigar_int_h[640*i+j],cigar_h[640*i+j]);
			}
			printf("]\n");
		}
		cudaFree(direction);
