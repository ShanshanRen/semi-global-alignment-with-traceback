...
struct NUM_ADD
{
  ...
  __global__ void  calculate_cigar( int size, char * data, NUM_ADD *num_add,int4 * result, int * direction, int * MM_global_s) //, char * resultaa
{
...
//	__shared__ int extend;
	...
	//	printf("%d %d \n",round,read_number);
		for(int i=0;i<round;i++)
		{
			...
			int MMM=0; 
                	//short mt=0; //
			short2 mt_gap_size_v;
			char read_base;
			if(threadIdx.x<round_size)
                        read_base=read_base_array[aaa+threadIdx.x];
			...
			int current_reference_id=0;
					...
					{	
					MM[0]=MM_global[current_reference_id];
					gap_h[0]=gap_h_global[current_reference_id];
					gap_size_h[0]=gap_h_size_global[current_reference_id];
					}
				...
				//	else
					...
					if(step_right>=gap_v)
					{
						...
						{
							M=gap_v;
							curmt.x=mt_gap_size_v.y;
							//curBtrack=gap_size_v;
						}
					MMM=MM[threadIdx.x];
					mt_gap_size_v.x=gap_size_h[threadIdx.x].y;
					direction_index[640*(j+aaa)+(threadIdx.x+aaa)]=curmt;
			//		printf("%d %d %d %d \n",640*j+(threadIdx.x+aaa),curmt.x,curmt.y,current_reference_id);
			//if(threadIdx.x==read_reference_number.x-3)

				if(threadIdx.x==round_size-1 && i<round-1)
				{
				MM_global[current_reference_id]=M;
				gap_h_global[current_reference_id]=step_right;
				short2 aab;
				aab.x=ki;
				aab.y=curmt.y;
				gap_h_size_global[current_reference_id]= aab;
				}
				...
				{	
					...
					{
						result_row=M;
						result_row_index=aaa+threadIdx.x;  //
					}
				//	printf("%d %d  %d  %d %d \n",read_reference_number.y,M,result_row,result_row_index,threadIdx.x);
				}

				//if(i==round-1 )   printf("%d  %d %d   %d \n", round,aaa,  current_reference_id,threadIdx.x+aa+1 );
                         	if(  i==round-1 && read_reference_number.x==(threadIdx.x+aaa+1))
                                {
						if(M>=result_col)
						{
							result_col=M;
  ...
  __global__ void  calculate_cigar_2( int size, int4 * result, char * cigar,int * cigar_int,int * direction) //, char * result
{
	...
	while(offset<size)
		...
		if( threadIdx.x==0)
			...
			//	printf("%d %d %d \n",(result4.x+result4.y)*640+result4.y,btr.x,btr.y );
				if(btr.x>0)
				{
					new_state='D';
					step_length=btr.x;
 ...
 /*       fscanf(file,"%d",&size);
   ...
   */
	 char data[200][1000];
		...
		int * MM_global;
		int * gap_v_global;
		int * gap_v_size_global;
		cudaMalloc((int **)&MM_global,size*(640*3*sizeof(int)));


		dim3 block(128);
		dim3 grid(size);
		clock_gettime(CLOCK_MONOTONIC_RAW,&start);
		calculate_cigar<<<grid,block>>> (size,data_d,num_add_d,result_d,direction,MM_global); //result
