...
#define Y_STEPS 8
#define BLOCK_SIZE 128   //blockDim.x
...
printf("1   1 1 1 %s", cudaGetErrorString(err));
  ...
  if (err != cudaSuccess)
        {
printf("runnnnnn  %s", cudaGetErrorString(err));
        }		

	//	printf("%d\n", size*sizeof(short4));
	//	cudaDeviceSynchronize();
		err=cudaMemcpy(result_h,result_d,size*sizeof(short4),cudaMemcpyDeviceToHost);
		err=cudaMemcpy(cigar_h,cigar,128*sizeof(char)*size, cudaMemcpyDeviceToHost);
		cudaMemcpy(cigar_int_h,cigar_int,128*sizeof(int)*size,cudaMemcpyDeviceToHost);		
	  if (err != cudaSuccess)
        {
		printf("4      %s", cudaGetErrorString(err));
        }
